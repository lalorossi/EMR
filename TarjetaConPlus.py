...
class Tarjeta():
	...
	def Recarga(self, monto):
		...
		if monto==196:
			...
			self._Saldo+=92
		self._Saldo=self._Saldo-(self._plus*5.75)
		self.setPlus()
		self._Saldo=trunc(self._Saldo, 2)
	...
	def PagarBoleto(self, colectivo):
		...
		if(self.EsTransbordo(hora, colectivo)==False):
			if (self.getSaldo() >= 5.75):
				...
				if (self._plus==0):
					self._plus+=1
					self.setUltimoViaje(colectivo, hora, 'Viaje plus')
					self.Formateo(colectivo, hora, 'Viaje plus')
					self.AgregarViaje()
					return True
				if (self._plus==1):
					self._plus+=1
					self.setUltimoViaje(colectivo, hora, 'Viaje ultra plus')
					self.Formateo(colectivo, hora, 'Viaje ultra plus')
					self.AgregarViaje()
					return True
				else:
					return False
		...
		if (self.EsTransbordo(self._hora, colectivo)==False):
			if self._hora >= self._Hora6 and self._hora <= self._Hora24:
				if self.getSaldo()>=2.90:
					...
					if (self._plus==0):
						self._plus+=1
						self.setUltimoViaje(colectivo, self._hora, 'Viaje plus')
						self.Formateo(colectivo, self._hora, 'Viaje plus')
						self.AgregarViaje()
						return True
					if (self._plus==1):
						self._plus+=1
						self.setUltimoViaje(colectivo, self._hora, 'Viaje ultra plus')
						self.Formateo(colectivo, self._hora, 'Viaje ultra plus')
						self.AgregarViaje()
						return True
					else:
						return False
